...
change_field () {
    ...
    if [[ "$FIELD_INDEX" = [1-7] ]]; then
        ...
        read FIELD_VALUE
        echo
        echo -n "Are you sure you want to change the value from $FIELD_VALUE? (y/n) "
        read INPUT2
        echo
        if [[ "$INPUT2" =~ [yY] ]]; then
            echo "Old data:"
            ...
            awk -v x="$BUSINESS_CODE" -v y="$FIELD_INDEX" -v z="$FIELD_VALUE" 'BEGIN {FS=OFS=","} $1==x {$y=z} 1' $TMPFILE > tmp && mv tmp $TMPFILE
            echo "New data:"
            echo
            awk_on_screen
            echo
